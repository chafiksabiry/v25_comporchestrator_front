  ...
  const checkCompanyLeadsForAutoCompletion = async () => {
    try {
      ...
      if (response.data.hasLeads && response.data.count > 0) {
        ...
        try {
          await axios.put(
            `${API_BASE_URL}/companies/onboarding/companies/${companyId}/onboarding/phases/2/steps/6`,
            { status: 'completed' }
          );
      ...
      if (hasGigs) {
        try {
          await axios.put(
            ...
            }/companies/onboarding/companies/${companyId}/onboarding/phases/2/steps/4`,
            { status: "completed" }
          );
          // Update local state to reflect the completed step
    ...
    try {
      ...
      if (hasLeads) {
        ...
        try {
          await axios.put(
            `${import.meta.env.VITE_COMPANY_API_URL
            }/companies/onboarding/companies/${companyId}/onboarding/phases/2/steps/6`,
            { status: "completed" }
          );
          // Update local state to reflect the completed step
        ...
        if (hasActiveGig) {
          try {
            ...
            const completeResponse = await axios.put(
              `${import.meta.env.VITE_COMPANY_API_URL
              }/companies/onboarding/companies/${companyId}/onboarding/complete-last`
            );
        ...
        // If no gigs are active and step 13 was previously completed, mark it as in_progress
          ...
          try {
            ...
            if (currentPhase >= 4) {
              await axios.put(
                `${import.meta.env.VITE_COMPANY_API_URL
                }/companies/onboarding/companies/${companyId}/onboarding/phases/4/steps/13`,
                { status: "in_progress" }
              );
            }
  ...
  const loadCompanyProgress = async () => {
    ...
    try {
      ...
      const response = await axios.get<OnboardingProgressResponse>(
        `${import.meta.env.VITE_COMPANY_API_URL
        }/companies/onboarding/companies/${companyId}/onboarding`
      );
  ...
  const handleStartStep = async (stepId: number) => {
    ...
    try {
      ...
      if (isStepCompleted) {
        ...
        await axios.put(
          `${import.meta.env.VITE_COMPANY_API_URL
          }/companies/onboarding/companies/${companyId}/onboarding/phases/${phaseId}/steps/${stepId}`,
          { status: "in_progress" }
        );
    ...
    if (canAccessPhase) {
      ...
      if (
        ...
        try {
          await axios.put(
            `${import.meta.env.VITE_COMPANY_API_URL
            }/companies/onboarding/companies/${companyId}/onboarding/current-phase`,
            { phase: newPhase }
          );
