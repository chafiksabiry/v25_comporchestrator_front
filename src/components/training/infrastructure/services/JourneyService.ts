...
export class JourneyService {
  ...
  /**
   * Persiste le thème du viewer REP dans methodologyData.
   */
  static async saveJourneyRepViewerTheme(
    journeyId: string,
    theme: string
  ): Promise<{ ok: boolean; error?: string }> {
    if (!isValidMongoId(String(journeyId || '').trim())) {
    ...
    try {
      ...
      const payload = {
        ...
        methodologyData: {
          ...prevMd,
          repViewerTheme: String(theme || 'harx-night'),
          repViewerThemeSavedAt: new Date().toISOString(),
        },
      };
      const response = (await ApiClient.put(`/training_journeys/${journeyId}`, payload)) as any;
      if (response?.data?.success === false) {
      ...
      console.warn('[JourneyService] saveJourneyRepViewerTheme:', e);
      return { ok: false, error: e?.message || 'Erreur réseau' };
    }
  }
}
