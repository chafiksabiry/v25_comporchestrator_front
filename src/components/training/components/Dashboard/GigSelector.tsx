...
export default function GigSelector({ companyId, industryFilter, industryName, onGigSelect, selectedGigId }: GigSelectorProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 rounded-[10px] border border-dashed border-gray-300 px-3.5 py-3">
        <Loader2 className="h-4 w-4 animate-spin text-harx-500" />
        <p className="text-[13px] text-gray-500">Loading gigs...</p>
      </div>
    );
  }
    ...
    <div className="w-full space-y-2">
      <div className="relative w-full">
        <select
          ...
          className="w-full cursor-pointer appearance-none rounded-[10px] border border-gray-300 py-[11px] pl-3.5 pr-10 text-sm text-gray-900 transition-all outline-none hover:border-gray-400 focus:border-harx-500 focus:ring-2 focus:ring-harx-500/20 disabled:cursor-not-allowed disabled:border-gray-200 disabled:text-gray-400"
        >
