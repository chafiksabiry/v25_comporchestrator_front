    ...
    return (
        <div ref={containerRef} className={`relative ${className}`}>
            ...
            <button
                ...
                className={`w-full flex items-center justify-between appearance-none bg-white border border-gray-100/80 text-gray-900 font-bold py-2.5 px-4 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/5 transition-all duration-300 shadow-sm hover:shadow-md cursor-pointer min-w-[240px] text-base tracking-tight ${triggerClassName}`}
            >
                <span className={selectedOption ? 'text-gray-900' : 'text-gray-400 font-medium italic'}>
                    {selectedOption ? selectedOption.name : placeholder}
                </span>
                <ChevronDown
                    className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-blue-600' : 'group-hover:translate-y-0.5'}`}
                />
            </button>
                            ...
                            safeOptions.map((option) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between px-4 py-2.5 mb-0.5 rounded-lg text-left transition-all duration-200 group ${value === option.id
