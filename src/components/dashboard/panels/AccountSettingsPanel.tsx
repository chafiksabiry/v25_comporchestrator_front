...
function AccountSettingsPanel() {
  ...
  const loadUser = async () => {
    ...
    try {
      ...
      const u = data?.data;
      const resolvedEmail = u?.email || Cookies.get('userEmail') || '';
      const resolvedName = u?.fullName || localStorage.getItem('userFullName') || '';
      ...
      const fallbackEmail = Cookies.get('userEmail') || localStorage.getItem('userEmail') || '';
      const fallbackName = localStorage.getItem('userFullName') || '';
      setEmail(fallbackEmail);
      setFullName(fallbackName);
      setEditingFullName(fallbackName);
      setPhone('');
  ...
  const handleSaveFullName = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('userProfileUpdated', { detail: { fullName: nextName } })
      );
  ...
  const handleRequestEmailCode = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        `${backendUrl}/api/users/${userId}/email/request-change`,
  ...
  const handleConfirmEmailChange = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post<ApiUserResponse>(
        `${backendUrl}/api/users/${userId}/email/confirm-change`,
        { code: emailCode.trim() }
      );
      const next = data?.data?.email || newEmail.trim();
      setEmail(next);
      Cookies.set('userEmail', next, { path: '/' });
      localStorage.setItem('userEmail', next);
      setNewEmail('');
      setEmailCode('');
  ...
  const handleRequestPasswordCode = async () => {
    ...
    setPwdLoading(true);
    try {
      await axios.post(`${backendUrl}/api/users/${userId}/password/request-change`);
  ...
  const handleConfirmPasswordChange = async (e: React.FormEvent) => {
    ...
    setPwdLoading(true);
    try {
      await axios.post(`${backendUrl}/api/users/${userId}/password/confirm-change`, {
  ...
  const handleRequestPhoneOtp = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${backendUrl}/api/users/${userId}/phone/request-change`, {
  ...
  const handleConfirmPhoneChange = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post<ApiUserResponse>(
        `${backendUrl}/api/users/${userId}/phone/confirm-change`,
        { otp: phoneOtp.trim() }
      );
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5 font-medium">
            Tout changement sensible est confirmé par email (Brevo) ou OTP SMS.
          </p>
        </div>
      </div>
      ...
      {/* Shield icon used for styling; suppress unused import warning */}
